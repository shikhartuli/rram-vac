% This test runs the different tests on the memory traces. It extracts the
% performance and energy gains and plots them
...
groupLabels = { 'CS', 'FE', 'DT', 'DT(C)', 'FE+DT', 'MM', 'Conv' };

tic;
for exp = 1:1:7
    if exp == 1
        memTrace = fopen('reportCS_full_32.csv');
        disp('Runnning CS');
    elseif exp == 2
        memTrace = fopen('reportFE.csv');
        disp('Running FE');
    elseif exp == 3
        memTrace = fopen('reportDT.csv');
        disp('Running DT');
    elseif exp == 4
        memTrace = fopen('reportDT_cached_2.csv');
        disp('Running DT (Cached)');
    elseif exp == 5
        memTrace = fopen('reportFE+DT.csv');
        disp('Running FE+DT');
    elseif exp == 6
        memTrace = fopen('reportMM_new.csv');
        disp('Running MM');
    elseif exp == 7
        memTrace = fopen('reportConv.csv');
        disp('Running Conv');
...
yyaxis right;
plot([1, 2, 3, 4, 5, 6, 7], (timesStack_norm(:, 2) - timesStack_norm(:, 1))*100, '-o')
