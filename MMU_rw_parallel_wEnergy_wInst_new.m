function [writeStallTime, readStallTime, totalTime, readE, writeE, totalEnergy_inst, writeTimes_inst] = MMU_rw_parallel_wEnergy_wInst_new(rwArray, mu, sigma, numBits, leftDiscard, rightDiscard, offsetMult, writeBufferSize, waitBufferSize, bitReadTime, switchDetectTime, writeVolt, I_LRS, I_HRS, readEnergyPerBit, bufferAccessEnergy, Clk_M_Value, base, debug)
...
writeTimes_inst = zeros(P_cycles+(4*writeBufferSize*8), 1);
totalEnergy_inst = zeros(P_cycles+(4*writeBufferSize*8), 1);
...
while i <= P_cycles+(4*writeBufferSize*8)
    ...
    if i <= P_cycles && rwArray{i, 1} == 'w'
        ...
        elseif waitBufferSize > 0 && size(waitBuffer{end, 2}, 1) == waitBufferSize
...
%         end
        if size(waitBuffer, 1) > 0 && size(waitBuffer{end, 2}, 1) > 0 && ismember(rwArray{i, 2}, waitBuffer{end, 2}(:, 1))
            ...
            if base == 0
                ...
                % totalEnergy_inst(i) = totalEnergy_inst(i) + bufferAccessEnergy;
            end
            if i == P_cycles
                if mod(time, Clk_P) == 0
                    flushWrite = (time/Clk_P + 1)*Clk_P;
        ...
        elseif ~isnan(Memory(rwArray{i, 2})) % size(Memory_end, 1) > 0 && ismember(rwArray{i, 2}, Memory_end(:, 1))
            ...
            elseif finishTime > 0
                ...
                % totalEnergy_inst(i) = totalEnergy_inst(i) + numBits * readEnergyPerBit;
