function output = MMU_rw_parallel(rwArray, mu, sigma, numBits, leftDiscard, rightDiscard, offsetMult, writeBufferSize, bitReadTime, switchDetectTime, maxWaitBufferSize, base)
...
while i <= P_cycles+(4*writeBufferSize*8)
    ...
    if debug == 0 && size(waitBuffer, 1) > 10
        for w = 1:1:size(waitBuffer, 1)
            if size(waitBuffer{w, 2}, 1) > bufferSizeReq
                bufferSizeReq = size(waitBuffer{w, 2}, 1);
            end
        end
    ...
    if i <= P_cycles && rwArray{i, 1} == 'w'
        ...
        elseif maxWaitBufferSize > 0 && size(waitBuffer{end, 2}, 1) == maxWaitBufferSize
...
% output = stallCycles;
% if flushWrite == 0
%     output = [stallCycles, Memory{end, 1}];
% else
%     output = [stallCycles, Memory{flushWrite, 1}];
% end
output = [stallCycles, flushWrite, bufferSizeReq];
end
