function [outputTime, readE, writeE, maxWaitBufferSize] = MMU_rw_serial_wEnergy(rwArray, mu, sigma, numBits, leftDiscard, rightDiscard, offsetMult, writeBufferSize, bitReadTime, switchDetectTime, writeVolt, I_LRS, I_HRS, readEnergyPerBit, bufferAccessEnergy, Clk_M_Value, base, debug)
...
while i <= P_cycles+(4*writeBufferSize*8*8)
    ...
    if debug == 0 && size(waitBuffer, 1) > 10
        for b = 1:1:size(waitBuffer, 1)
            if size(waitBuffer{b, 2}, 1) > maxWaitBufferSize
                maxWaitBufferSize = size(waitBuffer{b, 2}, 1);
            end
        end
...
%     global leftDiscard;
    ...
    if base == 1
        ...
        finishTime = size(writeBuffer{end, 2}, 1) * Clk_P;
    else
        for w = 1:size(writeBuffer{end, 2}, 1)
            wordToWrite = dec2bin(writeBuffer{end, 2}(w, 2), numBits);
            wordAddress = writeBuffer{end, 2}(w, 1);
            ...
            for j = 1:numBits
                ...
                if bitToWrite == wordInMemory(j)
                    ...
                    finishTime = finishTime + writeTime_test;
                    finishTimeArray = [finishTimeArray; writeTime_test];
                    if wordInMemory(j) == '0' && bitToWrite == '1'
