...
% parfor masterIDX = 1: (numIter*length(muArray))
%     [j, i] = ind2sub([length(muArray), numIter], masterIDX);
%     mu = muArray(j);
%     output = MMU(P_cycles, mu, sigma, leftDiscard, rightDiscard, offsetMult);
%     maxBufferArray(j ,i) = output;
%     % maxBufferArray(j, i) = output{1};
%     % BufferWithTimeArray{j} = output{2}(:, [1, 2]);
%     % writeTimes{j} = output{3};
% end

parfor i = 1:1:numIter
    parfor_function(i, muArray, P_cycles, sigma, leftDiscard, rightDiscard, offsetMult);
    disp(i);
end
...
function parfor_function(i, muArray, P_cycles, sigma, leftDiscard, rightDiscard, offsetMult)
