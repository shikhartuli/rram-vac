...
for i = 1:length(writeBufferSize)
    for j = 1:numIter
        ...
        rwArray = cell(15000, 2); % cell(length(cellArray{1, 1}), 2);
        for k = 1:15000 % length(cellArray{1, 1})
            rwArray{k, 1} = cellArray{1, 1}(k+5000);
            if cellArray{1, 1}(k+5000) == 'w'
                rwArray{k, 2} = [cellArray{1, 2}(k+5000), hex2dec(cellArray{1, 3}(k+5000))];
            else
                rwArray{k, 2} = cellArray{1, 2}(k+5000);
            end
        end
...
%         writeEnergy_inst_base = zeros(size(rwArray, 1), numIter);
        writeTimes_inst = [];
        writeEnergy_inst = [];
        writeTimes_inst_base = [];
        writeEnrgy_inst_base = [];
        [~, ~, ~, ~, ~, writeEnergy_inst, writeTimes_inst] = MMU_rw_parallel_wEnergy_wInst_new(rwArray, mu, sigma, numBits, leftDiscard, rightDiscard, offsetMult, writeBufferSize(i), waitBufferSize, bitReadTime, switchDetectTime, writeVolt, I_LRS, I_HRS, readEnergyPerBit, bufferAccessEnergy, Clk_M_Value, 0, debug);
        ...
        [~, ~, ~, ~, ~, writeEnergy_inst_base, writeTimes_inst_base] = MMU_rw_parallel_wEnergy_wInst_new(rwArray, mu, sigma, numBits, leftDiscard, rightDiscard, offsetMultBase, writeBufferSize(i), waitBufferSize, bitReadTime, switchDetectTime, writeVolt, I_LRS, I_HRS, readEnergyPerBit, bufferAccessEnergy, Clk_M_Value, 1, debug);
...
figure;
% scatter(1:1:size(rwArray, 1), writeTimes_inst(1:1:size(rwArray, 1)) * Clk_M_Value, '.')
scatter(inst1(40:end), writeTimes_inst_new(40:end) * Clk_M_Value, 5, 'filled')
hold on
plot(inst1(40:end), movmean(writeTimes_inst_new(40:end) * Clk_M_Value, [4, 0]), '--k')
% scatter(40:1:size(rwArray, 1), writeTimes_inst_base(40:40:size(rwArray, 1)) * Clk_M_Value, '.')
scatter(inst2(40:end), writeTimes_inst_base_new(40:end) * Clk_M_Value, 5, 'filled')
% plot(inst2(40:end), movmean(writeTimes_inst_base_new(40:end) * Clk_M_Value, 5), '--r')
hold off
xlabel('Instructions')
ylabel('Write Times (seconds)')
legend({sprintf('Parallel Write Proposed (%d bits)', numBits), 'Moving Average', sprintf('Parallel Write Reference (%d bits)', numBits)});

figure;
% scatter(40:1:size(rwArray, 1), writeEnergy_inst, '.')
scatter(inst3(40:end), writeEnergy_inst_new(40:end), 5, 'filled')
hold on
plot(inst3(40:end), movmean(writeEnergy_inst_new(40:end), [5, 0]), '--k')
% scatter(40:1:size(rwArray, 1), writeEnergy_inst_base, '.')
scatter(inst4(40:end), writeEnergy_inst_base_new(40:end), 5, 'filled')
plot(inst4(40:end), movmean(writeEnergy_inst_base_new(40:end), [5, 0]), '--k')
hold off
xlabel('Instructions')
